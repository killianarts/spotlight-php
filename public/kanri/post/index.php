...
<main class="ml-64 text-blue-50">
    ...
    <button id="make-post-button"
            ...
            _="on click set emailInput to #email-label toggle .hidden on me then toggle .hidden on #make-post-form
                js(me, emailInput) emailInput.focus() end"><?= lispify('Make post')?></button>

    <div id="make-post-form"
         _="on click from body
             ...
             if is_in_me == false and I do not match .hidden toggle .hidden on me toggle .hidden on #make-post-button end end"
         class="bg-blue-50 rounded-xs hidden relative shadow-md shadow-cyan-600/70 px-3 my-5
             border-l border-t border-l-red-700/30 border-t-red-700/30
             border-r border-b border-r-emerald-700/30 border-b-emerald-700/30
             ">
        <button class="p-1 absolute top-1 right-1" _="on mousedown toggle .hidden on #make-post-form toggle .hidden on #make-post-button">
        ...
        <fieldset class="flex rounded-xs">
            <legend class="pt-1 text-(--main-color) rounded-xs font-berkeley-uc-black uppercase
                           before:bg-conic before:from-blue-600 before:via-emerald-300 before:to-blue-700 before:bg-clip-text
                           before:text-transparent before:content-['#\'']"><?= lispify('Make post')?></legend>
